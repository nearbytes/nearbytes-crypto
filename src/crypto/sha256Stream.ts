...
interface InternalPool {
  ...
  acquire(): Promise<Sha256Stream>;
  closeForTesting(): Promise<void>;
}

let pool: InternalPool | null = null;

function readEnvCapacity(): number {
...
function createPool(capacity: number): InternalPool {
  ...
  const takeWorker = (): Promise<PooledWorker> => {
    if (closed) return Promise.reject(new Error('sha256 stream pool is closed'));
  ...
  const acquire = async (): Promise<Sha256Stream> => {
  ...
  const closeForTesting = async (): Promise<void> => {
    if (closed) return;
    closed = true;
 ...
 */
export async function acquireSha256Stream(): Promise<Sha256Stream> {
 ...
 */
export async function _closeSha256StreamPoolForTesting(): Promise<void> {
  const p = pool;
  pool = null;
  if (p) await p.closeForTesting();
}
