...
function autoSelectParallelism(totalBytes: number, requested: number | undefined): number {
  const hostCores = Math.max(1, availableParallelism());
  if (requested === 1) return 1;
  if (totalBytes < PARALLEL_THRESHOLD_BYTES) return 1;
...
export async function computeMerkleHash(
  ...
  try {
    ...
    throw new HashError(
      `Failed to compute Merkle hash: ${error instanceof Error ? error.message : 'unknown error'}`,
      error instanceof Error ? error : undefined,
    );
  }
}

function computeLeavesSerial(
  data: Uint8Array,
  totalBytes: number,
  leafSize: number,
  nLeaves: number,
...
async function computeLeavesParallel(
  data: Uint8Array,
  totalBytes: number,
  leafSize: number,
  nLeaves: number,
...
function runLeafWorker(
  ...
  return new Promise((resolve, reject) => {
    ...
    let received = false;
    worker.once('message', (msg: { hashes: ArrayBuffer }) => {
      received = true;
 ...
 */
function mthReduce(leafDigests: Uint8Array, startLeafIdx: number, endLeafIdx: number): Uint8Array {
  return mthReduceRecursive(leafDigests, startLeafIdx, endLeafIdx);
}

function mthReduceRecursive(
  leafDigests: Uint8Array,
  start: number,
  end: number,
): Uint8Array {
  const n = end - start;
